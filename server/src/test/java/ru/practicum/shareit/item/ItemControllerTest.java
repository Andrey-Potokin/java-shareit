...
public class ItemControllerTest {
    ...
    final NewItemRequest newItemDto = NewItemRequest.builder().name("name").available(false)
            .description("description").requestId(2L).build();

    final UpdateItemRequest updateItemDto = UpdateItemRequest.builder().id(1L).available(true)
    ...
    void findItemsByNameOrDescription() throws Exception {
        String searchText = "2";
        when(itemService.findItemsByNameOrDescription(searchText))
    ...
    void getItemById_NotFound() throws Exception {
        ...
        mvc.perform(get("/items/1")
                ...
                .andExpect(jsonPath("$.error").value("Ошибка поиска!"));
    }
}